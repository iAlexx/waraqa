# Phase 10 Completion Report — User Reports (Closure Hardening)
...
1. **Rate-limit identity** — primary bucket = HMAC of trusted proxy IP only (UA excluded); raw IP never stored; missing/malformed IP fail-closed (503).
2. **Resolution metadata** — stamp `resolvedAt`/`resolvedBy`/`resolutionSummary` only on enter closed; stay-closed edits preserve stamps; reopen clears stamps and keeps `lastResolutionSummary`.
3. **Audit reliability** — editorial status transitions write `audit-events` before persist; audit failure aborts update; recoverable sanitized `resolutionReason` in metadata; citizen `report_received` remains best-effort (documented).
4. **Request gates** — multipart/non-JSON rejected before body parse; Content-Length + max body size enforced (byte-capped stream read).
5. **Roadmap form gap** — separate `encountered`; optional `serviceCenter` validated against Transaction centers.
6. **Success integrity** — no forgeable `?sent=1`; client success state only.
7. **Serverless cleanup** — opportunistic cleanup awaited when selected; still non-fatal.
8. **Migration safety** — no `EXCEPTION WHEN others` on audit enums; Transaction FK `ON DELETE RESTRICT`; audit enum down irreversibility documented.
9. **ZIP hygiene** — deleted leaked local credential files and old review ZIP; V2 archive from tracked files only + scanned.

## Schema / MVP differences vs roadmap §12 / §14.13

| Roadmap field | Phase 10 decision |
| --- | --- |
| what appears incorrect | `message` (required) |
| what the user encountered | `encountered` (required) |
| reportedValue / suggestedValue | **Intentionally omitted** — collapsed into `message` + `encountered` |
...
- Fail-closed if IP untrustworthy → HTTP 503.
...
- Editorial transitions: actor + from/to status + sanitized `resolutionReason` when closing; write in `beforeChange`; failure → no silent terminal state.
...
- Service-center FK → `svc_centers`.
- Transaction FK → `ON DELETE RESTRICT` (immutable report history).
- Audit enum values: `ADD VALUE IF NOT EXISTS`; **down cannot safely remove enum values** — documented.

## Security model (delta)

- JSON-only public POST; early multipart/size reject; byte-capped body read.
- Overlong source URLs are rejected (never truncated into a different URL).
...
Public: CTA → form → service-center optional → valid submit → client success; no sensitive URL; honeypot success without persistence; forged `?sent=1` shows form not success; 360/1440/RTL/keyboard/overflow covered by E2E; no app console errors on happy path.
...
3. OS/browser Print Preview → Phase 13.

## Forensic self-audit

| Source | Result |
| --- | --- |
| Master Roadmap §12 public form | PASS (encountered + optional service center) |
| Master Roadmap §14.13 schema | PASS with documented MVP collapses above |
